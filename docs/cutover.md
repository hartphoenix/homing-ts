...
The final qualification candidate is commit `f05b673`: local `bun run check` passed with 131 active
tests and 21 PostgreSQL-gated skips; the committed Python package suite passed 11/11 and the
...
against the expanded schema; streaming age-encrypted backup, isolated restore, repeated migrations,
and new-image health/HTML checks passed. The encrypted rehearsal artifact SHA-256 was
`66467d1927bd53d17505b9f68d8881bbe7b4fca3a90cbb006ab9ce3c0b2e09fa` before automatic cleanup.
Production was untouched; package/server adversarial blockers were fixed and obsolete local
rollback was removed. The code is ready to deploy. A physical native-macOS install/wake test and
production canary remain post-deploy/manual gates: they do not block deployment readiness, but
they block declaring rollout proven.
