...
| Local v1 state | A failed v1 installation could be treated as a rollback install | change | It is cleanup residue. Remove its job, runtime/config/state, skill copies, logs, installer backups, and credential metadata; preserve the separate `com.homing.backup` job and its logs. |
...
| Native gate | A discarded VM or disposable-user run was required | change | Hart's production Mac is the canary because there are no other active users. Record only paths exercised there; do not claim unverified native branches. |
| Rollback | A local v1 search bundle or Django server restored all prior behavior | change | Local v1 is not a rollback path. Ordinary failure pauses/removes v2 locally and selects the prior TypeScript image; database restore is reserved for corruption. |
